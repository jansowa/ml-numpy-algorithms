...
class MultinomialNaiveBayes:
    ...
    @staticmethod
    def calculate_theta_j_y0(X_class_0: np.ndarray, vocab: np.ndarray, laplace_smoothing: float) -> dict:
        flatten = np.concatenate(X_class_0)
        denominator = len(flatten) + laplace_smoothing * len(vocab)
        nominator = np.array([len(flatten[flatten == word]) + laplace_smoothing for word in vocab])
        values = nominator / denominator
        return dict(zip(vocab, values))

    @staticmethod
    def calculate_theta_j_y1(X_class_1: np.ndarray, vocab: np.ndarray, laplace_smoothing: float) -> dict:
    ...
    def fit(self, X: np.ndarray, y: np.ndarray, laplace_smoothing: float = 0) -> None:
        ...
        self.__theta_j_y0 = MultinomialNaiveBayes.calculate_theta_j_y0(X[y == 0], self.__vocab, laplace_smoothing)
        self.__theta_j_y1 = MultinomialNaiveBayes.calculate_theta_j_y1(X[y == 1], self.__vocab, laplace_smoothing)
